...
/** Davomat (DavomatZoneMap) bilan bir xil — OpenStreetMap */
...
export function MobileRouteMap({
  ...
  const cleanPoints = useMemo(
    () =>
      points.filter((p) => validCoord(p.lat, p.lng)).map((p) => ({
        ...p,
        lat: Number(p.lat),
        lng: Number(p.lng),
      })),
  ...
  useEffect(() => {
    ...
    for (const p of cleanPoints) {
      if (p.kind === "start" || p.kind === "end") continue;
    ...
    window.setTimeout(() => map.invalidateSize(), 40);
  }, [cleanPoints, emptyCenter, emptyZoom]);
  ...
  return (
    <div className={className} style={{ height, position: "relative", minHeight: 360 }}>
      ...
      />
      {distanceLabel ? (
        <div className="pointer-events-none absolute left-3 top-3 z-[1000] rounded-lg border border-border bg-background/95 px-2.5 py-1.5 text-xs font-semibold shadow-sm">
          Yo‘l: {distanceLabel}
        </div>
      ) : null}
      {cleanPoints.length > 0 ? (
        <div className="pointer-events-none absolute right-3 top-3 z-[1000] flex flex-col gap-1 rounded-lg border border-border bg-background/95 px-2.5 py-1.5 text-[11px] shadow-sm">
        ...
        <div className="pointer-events-none absolute bottom-3 left-1/2 z-[1000] max-w-[90%] -translate-x-1/2 rounded-lg border border-border bg-background/95 px-3 py-1.5 text-center text-xs text-muted-foreground shadow-sm">
          {emptyHint}
        </div>
      ) : null}
    </div>
