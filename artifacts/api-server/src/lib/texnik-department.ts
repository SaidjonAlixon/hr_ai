...
export async function syncTexnikDepartmentAssignments(): Promise<number> {
  ...
  await db.execute(sql`
    ...
    SELECT 'Demo Texnik', 'texnik', 'texnik1', 'pass123', 'active', ${id}
    WHERE NOT EXISTS (SELECT 1 FROM users WHERE login = 'texnik1')
  `);
  await db.execute(sql`
    INSERT INTO users (full_name, role, login, password, status, department_id)
    SELECT 'Demo Texnik rahbari', 'texnik_rahbar', 'texnik_rahbar1', 'pass123', 'active', ${id}
    WHERE NOT EXISTS (SELECT 1 FROM users WHERE login = 'texnik_rahbar1')
  `);
  return id;
}
