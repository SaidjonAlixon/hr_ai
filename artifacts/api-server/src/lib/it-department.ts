...
export async function syncItDepartmentAssignments(): Promise<number> {
  ...
  await db.execute(sql`
    ...
    SELECT 'Demo IT mutaxassisi', 'it', 'it1', 'pass123', 'active', ${id}
    WHERE NOT EXISTS (SELECT 1 FROM users WHERE login = 'it1')
  `);
  await db.execute(sql`
    INSERT INTO users (full_name, role, login, password, status, department_id)
    SELECT 'Demo IT rahbari', 'it_rahbar', 'it_rahbar1', 'pass123', 'active', ${id}
    WHERE NOT EXISTS (SELECT 1 FROM users WHERE login = 'it_rahbar1')
  `);
  return id;
}
